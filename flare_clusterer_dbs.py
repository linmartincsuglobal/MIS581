...
class ClusterFlares:
    ...
    def __init__(self,
        ...
        if log_file:
            ...
            logging.basicConfig(level=logging.INFO,
                                format="%(asctime)s [%(levelname)s] %(message)s",
                                datefmt='%m-%d %H:%M',
                                handlers=[logging.FileHandler(output_dir+'debug.log'),
                                          logging.StreamHandler(sys.stdout)])
        ...
        if len(self.lonlats) == 1:
            ...
            db_centroids = [self.lonlats]
            self.db_idxs = [np.array([0])]


        else:
            # self.gmix = self.make_gmm(self.lonlats, len(self.clusters))
            self.dbscan = self.make_dbscan(self.lonlats,)  # Use DBSCAN instead of GMM

            # self.gm_idxs = self.getGMMIdxs3(self.lonlats, self.gmix)

            # self.gm_idxs = self.getGMMIdxs4(self.lonlats, self.gmix)
            # self.db_idxs, db_centroids = self.getDBIdxsCentroids(self.lonlats, self.dbscan)  # Use DBSCAN instead of GMM
            self.db_idxs, db_centroids = self.getDBIdxsCentroids2(self.lonlats, self.dbscan)  # Use DBSCAN instead of GMM


            # gmm_centroids = [[self.gmix.means_[i][0], self.gmix.means_[i][1]] for i in range(len(self.gmix.means_))]


        self.dbs_cluster_points_lol = [[db_centroids[i], self.db_idxs[i]] for i in range(len(db_centroids))]
        self.dbs_cluster_points_lol = [pair for pair in self.dbs_cluster_points_lol if len(pair[1]) > 4]
        ...
        if self.verbose:
            ...
            logging.info('# of DBSCAN clusters: {}'.format(len(db_centroids)))
            # logging.info('Len gmm cluster after filtering: {}'.format(len(self.gmm_cluster_points_lol)))
            logging.info('# of DBSCAN clusters after filtering: {}'.format(len(self.dbs_cluster_points_lol)))

        # logging.info(self.gmm_cluster_points_lol)
        count=0
        # for pair in self.gmm_cluster_points_lol:
        for pair in self.dbs_cluster_points_lol:
        ...
        self.dbs_cluster_points_dict = self.getDBSClusterPointsDict(self.dbs_cluster_points_lol, self.filteredDF)  # Use DBSCAN instead of GMM
    ...
    def getDBSClusterPointsDict(self, cluster_point_pair_lst, main_dataframe, utm_zone=None):
        """
        Create a dictionary containing key information about each cluster.
        ...
        for pair in cluster_point_pair_lst:
            if not pair[1]:
                logging.info('No points associated with DBSCAN cluster centroid:')
                logging.info(pair, '\n')
        clstr_pts_dict = {}
        cluster_no = 0
        for pair in cluster_point_pair_lst:
    ...
    def make_dbscan(self, points_lola):
        if self.verbose:
            logging.info('Fitting DBSCAN model to lon-lat points...')
            start = time.time()
        dbs = DBSCAN(eps=0.006, min_samples=4).fit(points_lola)
        if self.verbose:
            logging.info('{} seconds for fitting DBSCAN'.format(time.time()-start))
        return dbs
    ...
    def getDBIdxsCentroids(self, lonlats, dbs_model):
        dbs_idxs = []
        dbc_centroids = []
        labels = dbs_model.labels_
        n_clusters_ = len(set(labels)) - (1 if -1 in labels else 0)
        for c in range(n_clusters_):
            dbs_idxs.append(list(np.nonzero(labels == c)[0]))
            points_of_cluster = lonlats[labels==c,:]
            c_centroid = np.mean(points_of_cluster, axis=0)
            dbc_centroids.append(list(c_centroid))

        return dbs_idxs, dbc_centroids


    def getDBIdxsCentroids2(self, lonlats, dbsmodel):
        dbsDF = pd.DataFrame(data=np.hstack((lonlats, dbsmodel.labels_[:, None])), columns=['lon', 'lat', 'label'])

        lblgroup = dbsDF.groupby(['label'])
        dbs_idxs = list(lblgroup.groups.values())
        # dbs_idxs = [np.array(l) for l in dbs_idxs][1:]
        dbs_idxs = [list(l) for l in dbs_idxs][1:]

        gb = lblgroup.agg({'lon': ['mean'], 'lat': ['mean']})
        dbc_centroids = np.hstack((gb['lon']['mean'].values[:, None], gb['lat']['mean'].values[:, None]))
        dbc_centroids = [c.tolist() for c in dbc_centroids][1:]
